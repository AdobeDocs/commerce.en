...
>[!TIP]
>
>During development, you can start with Pipedream (Option A) and switch to a Runtime action (Option B) if you hit the free-tier invocation quota. Both options implement the same API contract.
...
- **Shared libraries** — An HTTP client for the external API and a config module for reading and writing `aio-lib-state`.
...
The agent gives you the option to create a detailed implementation plan or complete a direct implementation.
...
>[!IMPORTANT]
>
>Do not skip or reorder the steps. Steps 1–3 are prerequisites to deployment. The `aio app deploy` command fails if no credentials have been configured.
...
This command subscribes to the shipping webhook with the following settings:
...
Before starting the storefront integration, verify that you have the following:
...
>[!TIP]
>
>The prompt is detailed because the full API contract is already available from the backend agent. By referencing `@docs/STOREFRONT_API_SPEC.md`, the agent knows the exact endpoint URL, request and response format, and error handling. The explicit requirements list prevents the agent from inventing scope. Specifically, requesting plan mode triggers the phased workflow that helps you steer the implementation early.
...
>
>- Display PDP and Cart estimates only for logged-in shoppers (no zip code input needed).
...
| PDP integration | Displays "Get it by [date]" with optional countdown between price and description |
| Cart integration | Displays date range ("Thu, Mar 12 – Fri, Mar 13") in right column above order summary |
...
1. In a browser, log in to your shopper account.

   Delivery estimates on PDP and Cart require an authenticated session with a saved default shipping address.

1. Navigate to a product page, and verify the following results:
...
After you skip or complete testing, the agent prompts you to proceed to cleanup. Upon confirmation, the agent archives any documentation artifacts produced during implementation.
...
- **Add server-side caching:** Implement `aio-lib-state` caching in the BFF action to reduce calls to the external API for repeated origin and destination pairs.
