...
Any filterable product attribute can be used as a facet except stock status (`quantity_and_stock_status`). The *Add facets* panel lists the current facets and makes it easy to assign additional product attributes as facets. During this three-step process, an attribute is chosen to be used as a facet, properties are edited if needed, and the changes published to the storefront.

![Add Facets](assets/facets-add.png)

>[!NOTE]
>
>For information about product display using stock status, see [Manage out-of-stock products](manage-out-of-stock-products.md).
