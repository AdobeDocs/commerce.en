...
Ensure your environment meets these requirements before beginning the migration process.
...
- Check your current [!DNL Live Search] version.
...
- You are using the Luma theme or a theme that inherits from Luma with minimal customizations.
- You have not made custom modifications to PLP layouts or templates.
- You have not made custom JavaScript extensions that interact with search results.

**Standard product catalog**:

- All product attributes use standard source models (not custom source models).
- There are no custom product types that require special rendering logic.
- Your catalog uses standard facets and filtering only.

**Standard integrations**:

- You are not using Google Tag Manager (GTM) for analytics.
- You are not using third-party extensions that modify search behavior.
...
Configure the PLP widget in the Commerce Admin.
...
1. Expand the **[!UICONTROL Storefront Features]** section.
1. Set **[!UICONTROL Enable Product Listing Widgets]** to **Yes**.
1. Click **[!UICONTROL Save Config]**.
1. Flush the cache: **[!UICONTROL System]** > Tools > **[!UICONTROL Cache Management]** > **[!UICONTROL Flush Magento Cache]**.

### Step 3: Test on staging

Validate the migration in a non-production environment before going live.
...
Move the validated configuration to your live storefront.

**Role**: Merchant/Partner

1. Schedule deployment during the maintenance window if possible.
1. Follow your standard deployment process.
1. Enable the PLP widget in production using [Step 2: Enable the PLP widget](#step-2-enable-the-plp-widget).
1. Monitor for any issues immediately after deployment.
1. Have a rollback plan ready if critical issues arise.

### Step 5: Validate and monitor

Track search performance and customer experience after the migration.
...
The following scenarios require additional planning, custom development, or specialized support beyond the standard migration steps.
...
1. **Reconfigure facets**:
   - Go to **[!UICONTROL Marketing]** > SEO & Search > **[!UICONTROL Live Search]**.
...
- Consider migrating to [Adobe Analytics](https://business.adobe.com/products/adobe-analytics.html) if applicable.
...
Contact the appropriate resource based on your specific needs.
...
Find answers to common questions about migrating from the search adapter to the PLP widget.
