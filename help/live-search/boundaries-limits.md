...
- Stock status (`quantity_and_stock_status`) is not supported as a facet. You can use `inStock: 'true'` to filter out of stock products. This is supported out of the box in the `LiveSearchAdapter` module when "Display out of stock products" is set to "True" in the [!DNL Commerce] Admin. For information about managing the display of out-of-stock products, see [Manage out-of-stock products](manage-out-of-stock-products.md).
