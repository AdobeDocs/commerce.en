...
- To restrict customer groups using [category permissions](https://experienceleague.adobe.com/en/docs/commerce-admin/catalog/categories/category-permissions):
   - Products must be assigned to the root category. (**Note:** [update](install.md#updating-live-search) the dependency module `commerce-data-export` to version 103.4.0+ to remove this limitation.)
