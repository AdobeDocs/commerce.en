...
- **[Data Feed Sync Status page](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-feed-sync-status){target="_blank"}**—Check the feed export status for deployments connected with [!DNL Live Search], [!DNL Product Recommendations], or [!DNL Catalog Service]. This dashboard shows the feed export status for each feed including any errors encountered. A detail view displays feed export status for individual feed items.
...
> - [Manage synchronization to [!DNL Commerce Optimizer]](../aco-connector/data-sync-manage.md) — Verify catalog data sync and manually resync
