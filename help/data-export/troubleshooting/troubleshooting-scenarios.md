...
**Cause:** This is expected behavior, not an error. The message appears when a cron-triggered partial sync attempts to run while a full reindex or `saas:resync` is already in progress. The [!DNL SaaS Data Export] extension uses a feed lock mechanism to prevent conflicting concurrent sync operations.
