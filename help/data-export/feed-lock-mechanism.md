...
description: Learn how the feed lock mechanism prevents race conditions and ensures data integrity when multiple processes attempt to sync the same feed concurrently.
...
The [!DNL SaaS Data Export] extension uses a feed lock mechanism to prevent race conditions when multiple processes attempt to sync the same feed concurrently.
This can happen, for example, when a cron-triggered resync overlaps with a manual `saas:resync` CLI call.
...
1. If the lock is **not available**, the operation is skipped and logged. No data is lost - the next cron run picks up pending changes after the current process finishes.
